...
print("Written by Kasun De Costa")

print("\n you get 5 guessese to guess the number between 1 and 25")

#random number selector
def generate_rand_num():
    randomNumber = randrange(1,26)      #this pick a random number when you run the program
    return randomNumber
...
#checking input
tempRandNum = generate_rand_num()
if useGuess != tempRandNum:  
    while Num<=5:
        if useGuess > tempRandNum:
            print("Lower")
            #Num = Num+1
            ...
            replay = input("want to play again? (y/n): ")      #asking to play again
            ...
            if replay.lower() == "y":
              tempRandNum = generate_rand_num()
              #randomNumber = randrange(1,26)                #resetting the random number
        ...
        if Num ==5:                                                                               #stops asking once the number of guesses reaches 5 and promps next message
           print("You did not guess the number, the number was " + str(tempRandNum))             #number was not guessed message
